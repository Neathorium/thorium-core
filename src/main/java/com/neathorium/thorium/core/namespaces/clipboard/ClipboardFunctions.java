...
public interface ClipboardFunctions {
    ...
    private static Data<Boolean> copyToClipboardCore(ClipboardData<String> data, String message) {
        ...
        final var getResult = ClipboardFunctions.getFromClipboardCore(data);
        final var exception = getResult.EXCEPTION();
        final var status = StringUtils.isNotBlank(getResult.OBJECT()) && ExceptionFunctions.isNonException(exception);
